$(document).on('wb-ready.wb', function () {
    ...
    $('#button_save').on("click", function (e) {
        ...
        if (($('#edit_form').valid() && formIsChanged('#edit_form', form_original_data)) || (!needValidate && formIsChanged('#edit_form', form_original_data))) {
            var actionType, actionUrl;
            actionType = "PUT";
            actionUrl = getBaseString() + localizedName + "/api/treatments/minlethalcon/" + theTR.id;

            var formData = getFormData("#edit_form");
            formData.numOfCons = $('#minlethalcon_table').find('> tbody > tr').length;
    ...
    $('#button_continue').click(function (e) {
        e.preventDefault();
        var redirectUrl = getBaseString() + localizedName + "/treatments/analysis/waterchem?trId=" + theTR.id;
        confirmExitOrContinue(form_original_data, $.getMessage("i18n.continue_warning_message"), redirectUrl);
    });

    $('#minlethalcon_table').on('click', 'span.glyphicon-minus-sign', function (e) {
        e.preventDefault();
        $(this).closest("tr").remove();
        setForm();
        var allRows = $('#minlethalcon_table').find('> tbody > tr');
        if (allRows.length === 0) {
            addRow();
        } else {
            var lastRow = $('#minlethalcon_table').find('> tbody > tr').eq(allRows.length - 1);
    ...
    $('#minlethalcon_table').on('click', 'span.glyphicon-plus-sign', function (e) {
        e.preventDefault();
        addRow();
...
function assignNamesToTableRow() {
    ...
    var exposureName = "exposure";
    var minConIdName = "minlethalConId";
    ...
    allRows.each(function (i) {
        ...
        tds.eq(3).find("input").attr("name", mlcName + String(i));
        tds.eq(3).find("input").attr("id", mlcName + String(i));
        tds.eq(3).find("label").attr("for", mlcName + String(i));

        tds.eq(4).find("input").attr("name", nicloPercName + String(i));
        tds.eq(4).find("input").attr("id", nicloPercName + String(i));
        tds.eq(4).find("label").attr("for", nicloPercName + String(i));

        tds.eq(5).find("input").attr("name", exposureName + String(i));
        tds.eq(5).find("input").attr("id", exposureName + String(i));
        tds.eq(5).find("label").attr("for", exposureName + String(i));

        tds.eq(8).find("input").attr("name", minConIdName + String(i));
    });
}

function addRow() {
    var rowLength = $('#minlethalcon_table >tbody >tr').length + 1;
    ...
    var cell_04 = '<td class="text-center"><label for="#"></label><input type="text" class="input-sm" size="4" value="0" name="#" placeholder="" data-rule-number="true" min="0" max="999.9" maxlength="5" /></td>';
    var cell_05 = '<td class="text-center"><label for="#"></label><input type="text" class="input-sm" size="4" value="" name="#" placeholder="" data-rule-number="true" min="0" max="100" maxlength="4" /></td>';
    var cell_06 = '<td class="text-center"><label for="#"></label><input type="text" class="input-sm" size="4" value="0" name="#" placeholder="" data-rule-digits="true" min="0" max="99" maxlength="2" /></td>';
    ...
    $('#minlethalcon_table > tbody:last').append($.parseHTML(strHTML));
    ...
    var allRows = $('#minlethalcon_table').find('> tbody > tr');
    if (allRows.length > 1) {
        $('#minlethalcon_table').find('> tbody > tr').eq(allRows.length - 2).find('td').eq(7).html('');
    }
    setForm();
}
...
function setForm() {
    ...
    resetIndexToTableRow('#minlethalcon_table');
    addActionListener();
    addValidationListener('#edit_form');

    $.applyAwesomplete('#minlethalcon_table')
}
    ...
    allRows.each(function (i) {
        ...
        if (rowBranch != null && $.trim(rowBranch).length > 0 && rowStation != null && $.trim(rowStation).length > 0) {
            ...
            if (brId !== undefined && sId !== undefined) {
                ...
                showErrorMessage("Minimum Lethal Concentration " + (i +1) + ": " + $.getMessage("i18n.select_value_datalist_dc"), 10, "displayMessage");
                valid = false;
            }
        }
    });
...
function loadMinLethalConcentrations() {
    if (theTR.trMinLethalConcentrations.length > 0) {
        $.each(theTR.trMinLethalConcentrations, function (i) {

            if ($('#minlethalcon_table').find('> tbody > tr').length !== (i + 1)) {
                addRow();
            }
            var lastRow = $('#minlethalcon_table').find('>tbody > tr').last();
            ...
            tds.eq(3).find('input').val(this.mlc);
            tds.eq(4).find('input').val(this.niclosamide);
            tds.eq(5).find('input').val(this.exposure);
